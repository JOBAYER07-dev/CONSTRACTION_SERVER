...
const connectDB = async () => {
    ...
    try {
        ...
        const con = await mongoose_1.default.connect(dbUri, {
            dbName: 'constructiON',
...
const authenticateToken = async (req, res, next) => {
    try {
        ...
        const authDb = mongoose_1.default.connection.getClient().db('better-auth');
...
const STYLE_INSTRUCTIONS = {
    standard: 'Write as a professional structural cost estimation report with clear Markdown headers (###) and bullet points.',
    detailed: 'Write as a highly detailed technical breakdown, including brief reasoning for each material quantity (e.g. why that much cement/steel is needed for this area and building type), using Markdown headers (###) and bullet points.',
    summary: 'Write as a concise executive summary aimed at a non-technical client — plain language, short bullet points, minimal jargon.',
};
const LENGTH_INSTRUCTIONS = {
    short: 'Keep the entire response brief — around 120-180 words total. Only include the core material quantities and the total budget.',
    standard: 'Keep the entire response moderate in length — around 250-350 words total.',
    detailed: 'Provide a thorough response — around 450-600 words total, including a short breakdown explanation for each material category.',
};
const buildEstimatePrompt = (area, buildingType, location, promptStyle = 'standard', outputLength = 'standard') => {
    const styleInstruction = STYLE_INSTRUCTIONS[promptStyle] || STYLE_INSTRUCTIONS.standard;
    const lengthInstruction = LENGTH_INSTRUCTIONS[outputLength] || LENGTH_INSTRUCTIONS.standard;
 ...
 */
app.post('/api/ai/generate-estimate', authenticateToken, async (req, res) => {
    try {
        const { area, buildingType, location, promptStyle, outputLength } = req.body;
        if (!area || !buildingType || !location) {
            res.status(400).json({
                success: false,
                error: 'area, buildingType and location are required to generate an estimate',
            });
            return;
        }
        const prompt = buildEstimatePrompt(area, buildingType, location, promptStyle, outputLength);
...
app.post('/api/projects/add', authenticateToken, async (req, res) => {
    try {
        const { title, shortDescription, fullDescription, image, area, buildingType, location, userId, aiEstimate: preGeneratedEstimate, promptStyle, outputLength, } = req.body;
        if (!title ||
            !shortDescription ||
            !fullDescription ||
            !image ||
        ...
        let generatedText = preGeneratedEstimate;
        if (!generatedText ||
 ...
 */
const generateFollowUpSuggestions = async (userMessage, assistantReply) => {
    try {
        const suggestionCompletion = await groq.chat.completions.create({
            messages: [
                {
                    role: 'system',
                    content: 'You generate exactly 3 short, natural follow-up questions (max 8 words each) ' +
        ...
        try {
            ...
            if (!candidate) {
                console.error('Follow-up suggestions: no JSON found in model output:', raw);
                return [];
            }
